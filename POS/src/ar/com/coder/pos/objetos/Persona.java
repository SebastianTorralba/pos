package ar.com.coder.pos.objetos;

/***********************************************************************
 * Module:  Persona.java
 * Author:  Sebastian
 * Purpose: Defines the Class Persona
 ***********************************************************************/

import java.util.*;

/** @pdOid 505baa28-5f5f-4ab9-92b5-ab59ba2ac375 */
public class Persona {
   /** @pdOid ec1252a1-f830-46d3-a2e7-51d529e543cb */
   private java.lang.String codPersona;
   /** @pdOid 40dce5b1-59e5-461f-a47f-de114a60fd38 */
   private java.lang.String denominacion;
   /** @pdOid b2f1919d-460c-4971-b01e-f94acdd86420 */
   private java.lang.String cuit;
   /** @pdOid 65d59b04-96de-4870-b026-676106e2144e */
   private java.util.Date fechaAlta;
   /** @pdOid 34cb52f6-6416-480a-94d3-8e565e3f47d8 */
   private java.lang.String direccion;
   /** @pdOid 8092c915-4f4c-4edd-9ab7-761e5678bc7a */
   private java.lang.String contacto;
   /** @pdOid c9425b2a-0edb-4bb8-bebe-a6f4dcb2745c */
   private java.lang.String telefono;
   /** @pdOid fe5245a4-cc09-4fb9-9577-0af9a90f1b5f */
   private java.lang.String celular;
   /** @pdOid a67571aa-3d9c-42fb-b012-891aee483cb7 */
   public java.lang.String mail;
   /** @pdOid 365ca11a-6d44-48b1-b6df-9e5e9fa72fe7 */
   private java.util.Date fechaRegistro;
   /** @pdOid 78b03b91-68fa-4b9b-875d-448ada934cf9 */
   private java.lang.String usuario;
   
   /** @pdOid 2fe18704-9f8a-45d9-8c36-067ef8b2192f */
   public java.lang.String getCodPersona() {
      return codPersona;
   }
   
   /** @param newCodPersona
    * @pdOid 5cbe97c9-8965-4a98-816d-0fe9d7b792fc */
   public void setCodPersona(java.lang.String newCodPersona) {
      codPersona = newCodPersona;
   }
   
   /** @pdOid 44da9941-bace-4c75-a311-e84bd5a70721 */
   public java.lang.String getDenominacion() {
      return denominacion;
   }
   
   /** @param newDenominacion
    * @pdOid a76f189b-27bb-40d3-9fed-6f00b55ef62f */
   public void setDenominacion(java.lang.String newDenominacion) {
      denominacion = newDenominacion;
   }
   
   /** @pdOid 786be4e1-1734-4908-bb71-92f8e8ed88ad */
   public java.lang.String getCuit() {
      return cuit;
   }
   
   /** @param newCuit
    * @pdOid 9da2d37e-af25-4a02-bea5-f572705506a5 */
   public void setCuit(java.lang.String newCuit) {
      cuit = newCuit;
   }
   
   /** @pdOid 0c368623-deac-4b32-9147-6c3ed813e71a */
   public java.util.Date getFechaAlta() {
      return fechaAlta;
   }
   
   /** @param newFechaAlta
    * @pdOid 4f041dc3-d149-40c4-a5b6-b4eda93b2627 */
   public void setFechaAlta(java.util.Date newFechaAlta) {
      fechaAlta = newFechaAlta;
   }
   
   /** @pdOid d8bcd91c-6236-4ccb-bfc6-590640301980 */
   public java.lang.String getDireccion() {
      return direccion;
   }
   
   /** @param newDireccion
    * @pdOid 4abc7363-9c95-4bd3-8d05-24ee6093f8e0 */
   public void setDireccion(java.lang.String newDireccion) {
      direccion = newDireccion;
   }
   
   /** @pdOid aeb7b358-5ee9-4f4f-a962-376438e32825 */
   public java.lang.String getContacto() {
      return contacto;
   }
   
   /** @param newContacto
    * @pdOid 1402460a-7872-435f-a877-7dfbf367f6ab */
   public void setContacto(java.lang.String newContacto) {
      contacto = newContacto;
   }
   
   /** @pdOid 6e82c9ae-77f8-4e5e-8f4c-714a8e82f415 */
   public java.lang.String getTelefono() {
      return telefono;
   }
   
   /** @param newTelefono
    * @pdOid 73fde092-23be-4f17-b9d2-3cfe5fa0987d */
   public void setTelefono(java.lang.String newTelefono) {
      telefono = newTelefono;
   }
   
   /** @pdOid 2a78b82d-d3a2-476b-b822-bdc499df487f */
   public java.lang.String getCelular() {
      return celular;
   }
   
   /** @param newCelular
    * @pdOid ae6f0d51-eec2-4e61-8a6b-3697923c005b */
   public void setCelular(java.lang.String newCelular) {
      celular = newCelular;
   }
   
   /** @pdOid 47d59275-e7cf-49ae-bfe6-bf161358cdde */
   public java.lang.String getMail() {
      return mail;
   }
   
   /** @param newMail
    * @pdOid 05d6d6a0-a272-4a43-833e-40139e668bb8 */
   public void setMail(java.lang.String newMail) {
      mail = newMail;
   }
   
   /** @pdOid bbc1b05e-de88-4f94-805b-7cde8d74848a */
   public java.util.Date getFechaRegistro() {
      return fechaRegistro;
   }
   
   /** @param newFechaRegistro
    * @pdOid 799af716-fc58-4a86-9978-2551ea70d0dc */
   public void setFechaRegistro(java.util.Date newFechaRegistro) {
      fechaRegistro = newFechaRegistro;
   }
   
   /** @pdOid cd9bcc5a-6745-4d07-95c1-e3db038d9bf1 */
   public java.lang.String getUsuario() {
      return usuario;
   }
   
   /** @param newUsuario
    * @pdOid c1acc700-c00e-4791-a125-587d78fa2b3e */
   public void setUsuario(java.lang.String newUsuario) {
      usuario = newUsuario;
   }

}